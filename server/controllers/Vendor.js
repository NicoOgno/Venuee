...
exports.registerVendor = async (req, res) => {
    try {
    const { businessName, email, password, streetAddress, city, state, zipCode, maxCapacity, vendorImg  } = req.body;
    ...
    if (vendor) {
        ...
        res.status(400).send({ error, message: 'Could not create new vendor' });
    }
};
...
exports.vendorLogin = async (req, res) => {
    ...
    try {
        ...
        console.log('ERROR', error);
        res.status(400).send({ error: '401', message: 'Username or password is incorrect' });
    }
};
